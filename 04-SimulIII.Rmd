...
X11=rnorm(n,mean=1,sd=vary)
X12=rnorm(n,mean=0,sd=vary)
A1=rbinom(n, 1, expit(1.5*X12+2*X11))
A1_Opt=((0.5*X11+0.5*X12+0.1*XX)>0.5)+0

X21=rnorm(n,0+0.2*A1+0.1*X11,sd=vary)
X22=rnorm(n,0+0.5*A1+0.1*X12,sd=vary)
A2=rbinom(n, 1, expit(1.5*X22-0.6*A1+ 2*X21))
A2_Opt=((0.5*X21+0.5*X22+0.1*XX)>0.5)+0

X31=rnorm(n,0+0.2*A2+0.1*X21,sd=vary)
X32=rnorm(n,0+0.5*A2+0.1*X22,sd=vary)
A3=rbinom(n, 1, expit(1.5*X32-0.6*A2+ 2*X31))
A3_Opt=((0.5*X31+0.5*X32+0.1*XX)>0.5)+0

X41=rnorm(n,0+0.2*A3+0.1*X31,sd=vary)
X42=rnorm(n,0+0.5*A3+0.1*X32,sd=vary)
A4=rbinom(n, 1, expit(1.5*X42-0.6*A3+ 2*X41))
...
for(i in 1:nrow(theta)){
  ...
  estimate[i]=mean(X11+X12- (0.5*X11+0.5*X12+0.1*XX-0.5)*(A1_Opt-A1_regime)-
                            (0.5*X21+0.5*X22+0.1*XX-0.5)*(A2_Opt-A2_regime)-
                            (0.5*X31+0.5*X32+0.1*XX-0.5)*(A3_Opt-A3_regime)-
                            (0.5*X41+0.5*X42+0.1*XX-0.5)*(A4_Opt-A4_regime))
}
# 
indy=which(estimate==max(estimate))
theta[indy,]
...
plot_ly(
  ...
  layout(title="Value of Regime for Varying Cutoffs \n IPW",
         scene=list(xaxis = list(title="Psi1"),
                    yaxis = list(title="Psi3"),
                    zaxis =  list(title="Y")))
...
#----- Reading in data
pathy="C:/Users/Administrator/Dropbox/Daniel_PhD_Research/Objective II/Code/ComputerExperiments/Simulation III/"
TabyVal_mean=read.csv(file=paste0(pathy,"SimulationIII_TablesFigures/TabyVal_mean_n1000Base_NoiseHomosk.csv"),row.names=1)
Tabypsi1_mean=read.csv(file=paste0(pathy,"SimulationIII_TablesFigures/Tabypsi1_mean_n1000Base_NoiseHomosk.csv"),row.names=1)
Tabypsi2_mean=read.csv(file=paste0(pathy,"SimulationIII_TablesFigures/Tabypsi2_mean_n1000Base_NoiseHomosk.csv"),row.names=1)

TabyVal_median=read.csv(file=paste0(pathy,"SimulationIII_TablesFigures/TabyVal_median_n1000Base_NoiseHomosk.csv"),row.names=1)
Tabypsi1_median=read.csv(file=paste0(pathy,"SimulationIII_TablesFigures/Tabypsi1_median_n1000Base_NoiseHomosk.csv"),row.names=1)
Tabypsi2_median=read.csv(file=paste0(pathy,"SimulationIII_TablesFigures/Tabypsi2_median_n1000Base_NoiseHomosk.csv"),row.names=1)
